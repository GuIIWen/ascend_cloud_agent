...
## 2026-03-26 17:29:10 +0800

### 主题
P10 验收 P8 修复：统一清洗 generated testcase 的 requiredConfig 值

### 参与角色
- P8：补强 `GeneratedTestcasePostProcessor`，修复 `requiredConfig` 值清洗缺口
- P10 主线程：代码范围审查、测试验收、live 验证

### 验收结论
- 验收通过，收这个提交。
- 本次修复的缺口是真实存在的：
  - 之前 `requiredConfig(...)` 不会统一做 `trim` 和 `CR/LF` 清理
  - 脏 token 会直接进入 `X-Auth-Token` 请求头
  - 这和此前真实运行中出现过的 `invalid header value` 风险一致
- 本次修复方式也正确：
  - 不是只改 prompt，而是在 `GeneratedTestcasePostProcessor` 里统一规范化 `requiredConfig(...)`
  - 连已有自定义 `requiredConfig(...)` 方法体也会被收敛成统一实现

### 关键证据
- 提交：`6ceb22f fix: sanitize generated required config values`
- 提交范围仅 2 个文件：
  - `src/main/java/com/agent/service/testcase/GeneratedTestcasePostProcessor.java`
  - `src/test/java/com/agent/service/testcase/GeneratedTestcasePostProcessorTest.java`
- 主线程验收测试：
  - `mvn -q -Dtest=GeneratedTestcasePostProcessorTest,TestcaseGenerationServiceImplTest test` => 通过
- 主线程 live 验证：
  - 服务已重启并健康：`GET /actuator/health => UP`
  - `POST /api/testcase/generate` 返回的生成代码中，`requiredConfig(...)` 已包含：
