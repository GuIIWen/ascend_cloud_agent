...
1. **评审后必须留痕**
   - 对架构评审、代码审查、缺陷分析、技术决策类任务，主Agent必须产出或更新根目录 `meeting.md`
   - 纪要必须包含评审时间、范围、核心问题、分析结论、决策、行动项
   - 默认采用追加模式保留历史记录，不覆盖已有纪要

2. **问题分级**
   - 使用 `P0 / P1 / P2` 标记问题优先级
   - 明确区分“确定 bug”、“架构风险”、“测试覆盖缺口”

3. **结论要求**
   - 需要记录最终统一结论，而不是只堆叠子Agent原始输出
   - 需要注明关键证据文件，便于事后追溯

4. **提交纪律**
   - 一轮变更完成并通过最小必要验证后，优先提交一个非交互 `git commit`
   - 后续继续修改前，尽量以上一个已提交状态作为新的工作起点
